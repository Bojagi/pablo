...
import { getColor, getSpacing } from '../styleHelpers/index.ts';
...
You can use theme values by using functions like you would in `styled-components`. For easier access
Pablo provides helper functions like `getColor` or `getSpacing`. For usage of those take a look at the
example below.
...
const customComponentStyles = { 
  checkbox: {
    ...
    handleColor: getColor('brand', 'light'),
  },
  button: {
    base: {
      icon: {
    ...
    brand: {
      outlineColor: getColor('brand', 'darkest'),
    },
    sizes: {
      small: {
        padding: css`
