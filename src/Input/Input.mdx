...
The simples form of an `Input` is just the field itself. You can just pass a `useState` setter function to update state
...
A simple input field doesn't really make sense as the context is missing. You can pass the `label` prop with the
