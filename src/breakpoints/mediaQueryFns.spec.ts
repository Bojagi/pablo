...
test('show above mediaQuery with mediaQueryAbove()', () => {
  const query = mediaQueryAbove(['100px', '200px', '300px', '400px'], 1);
  expect(query).toBe('only screen and (min-width: 200px)');
});

test('show below mediaQuery with mediaQueryAbove()', () => {
  const query = mediaQueryBelow(['100px', '200px', '300px', '400px'], 2);
  expect(query).toBe('only screen and (max-width: calc(300px - 1px))');
});

test('show above mediaQuery with mediaQueryOnly() when index is length - 1', () => {
  const query = mediaQueryOnly(['100px', '200px', '300px', '400px'], 3);
...
test('show between media query with mediaQueryOnly() when index is between first and last', () => {
  const query1 = mediaQueryOnly(['100px', '200px', '300px', '400px'], 0);
  expect(query1).toBe('only screen and (min-width: 100px) and (max-width: calc(200px - 1px))');
  const query2 = mediaQueryOnly(['100px', '200px', '300px', '400px'], 1);
  expect(query2).toBe('only screen and (min-width: 200px) and (max-width: calc(300px - 1px))');
  const query3 = mediaQueryOnly(['100px', '200px', '300px', '400px'], 2);
